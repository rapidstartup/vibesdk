  ...
  export abstract class BaseSandboxService {
    ...
    static async listTemplates(): Promise<TemplateListResponse> {
        try {
            ...
            // 2) Fallback to GitHub raw catalog if R2 missing
            const repo = env.TEMPLATES_REPOSITORY || 'https://raw.githubusercontent.com/cloudflare/vibesdk-templates/main';
            const catalogUrl = `${repo.replace(/\.git$/, '')}/template_catalog.json`;
            const ghResp = await fetch(catalogUrl);
            if (!ghResp.ok) {
                throw new Error(`Failed to fetch template catalog from fallback: ${ghResp.status} ${ghResp.statusText}`);
            }
            const templates = await ghResp.json() as TemplateInfo[];
            const filteredTemplates = templates.filter(t => !t.name.includes('next'));
            return {
                success: true,
                templates: filteredTemplates.map(t => ({
