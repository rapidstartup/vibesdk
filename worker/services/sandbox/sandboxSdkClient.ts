...
export class SandboxSdkClient extends BaseSandboxService {
    ...
    private async waitForServerReady(instanceId: string, processId: string, maxWaitTimeMs: number = 10000): Promise<boolean> {
        ...
        const readinessPatterns = [
            ...
            /Available on:/i             // http-server output
        ];
    ...
    private async startDevServer(instanceId: string, port: number): Promise<string> {
        try {
            // Detect framework to construct a compatible dev command
            const sandbox = this.getSandbox();
            let devCmd = 'bun run dev';
            try {
                const pkgFile = await sandbox.readFile(`${instanceId}/package.json`);
                if (pkgFile.success) {
                    const pkg = JSON.parse(pkgFile.content || '{}');
                    const scripts = pkg.scripts || {};
                    const deps = { ...(pkg.dependencies || {}), ...(pkg.devDependencies || {}) } as Record<string, string>;
                    const devScript: string = scripts.dev || '';
                    const usesNext = /next\s+dev|next-dev|\bnext\b/i.test(devScript) || 'next' in deps;
                    const usesVite = /vite\s|\bvite\b/i.test(devScript) || 'vite' in deps;
                    const hasDevScript = typeof devScript === 'string' && devScript.trim().length > 0;
                    if (usesNext) {
                        devCmd = hasDevScript
                    ...
                    // Ensure an index.html exists for Vite/React apps; create a minimal one if missing
                    try {
                        const idx = await sandbox.readFile(`${instanceId}/index.html`);
                        if (!idx.success || !idx.content || idx.content.trim().length === 0) {
                            const minimal = `<!doctype html>\n<html>\n  <head>\n    <meta charset="UTF-8" />\n    <meta name="viewport" content="width=device-width, initial-scale=1.0" />\n    <title>Preview</title>\n  </head>\n  <body>\n    <div id="root"></div>\n    <script type="module" src="/src/main.tsx"></script>\n  </body>\n</html>\n`;
                            await sandbox.writeFile(`${instanceId}/index.html`, minimal);
                            this.logger.info('Injected minimal index.html for preview');
                        }
                    } catch (e) {
                        const minimal = `<!doctype html>\n<html>\n  <head>\n    <meta charset="UTF-8" />\n    <meta name="viewport" content="width=device-width, initial-scale=1.0" />\n    <title>Preview</title>\n  </head>\n  <body>\n    <div id="root"></div>\n    <script type="module" src="/src/main.tsx"></script>\n  </body>\n</html>\n`;
                        await sandbox.writeFile(`${instanceId}/index.html`, minimal);
                        this.logger.info('Injected minimal index.html for preview (catch path)');
                    }
                }
            } catch (e) {
                // Fallback to generic dev command
            }

            // Allow forcing a specific dev command via environment
            try {
                const forced = (this as any).env?.SANDBOX_DEV_CMD as string | undefined;
                if (forced && forced.trim().length > 0) {
                    devCmd = forced.replace(/\$PORT/g, String(port)).replace(/\$HOST/g, '0.0.0.0');
                }
            } catch (e) {
                this.logger.debug?.('SANDBOX_DEV_CMD check failed', { error: e instanceof Error ? e.message : String(e) });
            }

            // Pipe stdout/stderr to files; set PORT envs; dev-first, then build+static fallback, and support static-only mode
            const command = `bash -lc '
                set -e
                export PORT=${port} VITE_PORT=${port} HOST=0.0.0.0 BROWSER=none \
                       ROOT_HINT="${(this as any).env?.SANDBOX_STATIC_ROOT || ''}" \
                       PREVIEW_MODE="${(this as any).env?.SANDBOX_PREVIEW_MODE || ''}"

                build_static() {
                  if [ -f package.json ]; then
                    echo "[fallback] building static assets" >> .dev-stderr.log
                    (bun run build || bunx --yes vite build || true) >> .dev-stdout.log 2>> .dev-stderr.log || true
                  fi
                }

                start_static() {
                  ROOT_DIR="."
                  if [ -n "$ROOT_HINT" ] && [ -f "$ROOT_HINT/index.html" ]; then
                    ROOT_DIR="$ROOT_HINT"
                  else
                  ...
                  if command -v bunx >/dev/null 2>&1 && bunx --yes http-server -v >/dev/null 2>&1; then
                    bunx --yes http-server "$ROOT_DIR" -p ${port} -a 0.0.0.0 >> .dev-stdout.log 2>> .dev-stderr.log &
                  elif command -v bunx >/dev/null 2>&1 && bunx --yes serve -h >/dev/null 2>&1; then
                    bunx --yes serve -s -l ${port} "$ROOT_DIR" >> .dev-stdout.log 2>> .dev-stderr.log &
                  fi
                }
                ...
                ( ${devCmd} ) > .dev-stdout.log 2> .dev-stderr.log &
                DEV_PID=$!
                # Give it some time; the Worker will poll readiness separately
                sleep 6 || true
                # Background watchdog to start a static server if nothing binds
                (
                  sleep 18 || true
                  if ! ss -ltn 2>/dev/null | grep -q ":${port} "; then
                    echo "[fallback] primary dev server not listening; running static build and serving on ${port}" >> .dev-stderr.log
                    build_static
                    start_static
                  fi
                ) &
                ...
                wait $DEV_PID || true
            '`;
            const process = await sandbox.startProcess(command, { cwd: instanceId });
            this.logger.info('Development server started', { instanceId, processId: process.id, port });

            // Best-effort readiness (non-blocking). If readiness check fails, continue.
            try {
                const isReady = await this.waitForServerReady(instanceId, process.id, 25000);
                if (isReady) {
                    this.logger.info('Development server is ready', { instanceId });
                } else {
                    this.logger.warn('Development server may not be fully ready yet', { instanceId });
                }
            } catch (readinessError) {
                this.logger.warn('Readiness check error (continuing):', readinessError);
            }
            ...
            if (installResult.exitCode === 0) {
                ...
                try {
                    ...
                    if (previewDomain) {
                        ...
                        // Ensure subdomain format is <port>-<id>.<domain> with new domain
                        // Older previews might still reference the previous domain; normalization above fixes that
                    }
    ...
    async createInstance(templateName: string, projectName: string, webhookUrl?: string, localEnvVars?: Record<string, string>): Promise<BootstrapResponse> {
        try {
            ...
            // Sensible defaults
            (this as any).env = (this as any).env || {};
            // Run setup commands by default so dev server has its deps
            if (!(this as any).env.SANDBOX_SKIP_SETUP_COMMANDS) {
                (this as any).env.SANDBOX_SKIP_SETUP_COMMANDS = 'false';
            }
            if (!(this as any).env.SANDBOX_STATIC_ROOT) {
                (this as any).env.SANDBOX_STATIC_ROOT = 'dist/client';
            }
    ...
    // LOG RETRIEVAL
    // ==========================================
    async getLogs(instanceId: string, _onlyRecent?: boolean): Promise<GetLogsResponse> {
        try {
            this.logger.info('Retrieving instance logs', { instanceId });
            // Fallback approach: read vite log file if present, else return empty logs
            const sandbox = this.getSandbox();
            const stdoutFile = `${instanceId}/.dev-stdout.log`;
            const stderrFile = `${instanceId}/.dev-stderr.log`;

            const [out, err] = await Promise.all([
                sandbox.readFile(stdoutFile).catch(() => ({ success: false, content: '' } as any)),
                sandbox.readFile(stderrFile).catch(() => ({ success: false, content: '' } as any)),
            ]);

            return {
                success: true,
                logs: {
                    stdout: out?.success ? out.content : '',
                    stderr: err?.success ? err.content : ''
                },
                error: undefined
            };
        } catch (error) {
    ...
    async getInstanceErrors(instanceId: string, _clear?: boolean): Promise<RuntimeErrorResponse> {
        try {
            // No monitor-cli: attempt to read recent stderr content and parse common error lines
            const sandbox = this.getSandbox();
            const stderrFile = `${instanceId}/.dev-stderr.log`;
            const read = await sandbox.readFile(stderrFile).catch(() => ({ success: false, content: '' } as any));
            const content: string = read?.success ? read.content : '';
            const sanitized = Array.from(content).map(ch => {
                const code = ch.charCodeAt(0);
                return code < 32 ? ' ' : ch;
            }).join('');
            const lines: string[] = sanitized.split('\n').slice(-200);
            const errors: RuntimeError[] = lines
                .filter((line: string) => /error|failed|exception|trace/i.test(line))
                .map((line: string) => ({ timestamp: new Date(), message: line.slice(0, 512), stack: line, severity: 'error', source: 'stderr', rawOutput: line }));
            ...
            return { success: false, errors: [], hasErrors: false, error: `Failed to get instance errors: ${error instanceof Error ? error.message : 'Unknown error'}` } as any;
        }
    }
    ...
    private async createLatestCommit(instanceId: string, commitMessage: string): Promise<string> {
        ...
        const sanitizedMessage = Array.from(commitMessage.substring(0, 500))
            .map(ch => {
                const code = ch.charCodeAt(0);
